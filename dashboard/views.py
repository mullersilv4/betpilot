...
PROTECTION_STRATEGIES = {'Surebet', 'Proteção', 'Arbitragem', 'Extração de freebet', 'Proteção do duplo'}
...
def protection_winners_net_result(bet, entries, winners):
    winners = list(winners)
    if not winners:
        return Decimal('0.00')
    if len(winners) == 1 and bet.strategy != 'Extração de freebet':
        return winners[0].net_result
    if bet.strategy == 'Extração de freebet':
        return sum(
            (entry.settlement_result_for_winners(winners) for entry in entries),
            start=Decimal('0.00'),
...
def apply_manual_protection_winners(bet, entries, winners, user):
    winners = list(winners)
    ...
    with transaction.atomic():
        ...
        bet.actual_net_result = protection_winners_net_result(bet, entries, winners)
        bet.status = Bet.Status.WON if bet.actual_net_result >= 0 else Bet.Status.LOST
        bet.exact_score = ' + '.join(f'{winner.bookmaker} - {winner.label}' for winner in winners)[:40]
        bet.save(update_fields=['actual_net_result', 'status', 'exact_score'])
        create_protection_balance_movements(bet, entries, winners)
        ...
        for entry in entries:
            ...
            should_create_freebet = (
                ...
                or (
                    ...
                    and entry.pk in winner_ids
                )
                or (
                    entry.freebet_trigger == SureBetEntry.FreeBetTrigger.LOST
                    and entry.pk not in winner_ids
                )
            )
...
def selected_winner_signatures(bet, entry_ids):
    winners = bet.surebet_entries.filter(pk__in=entry_ids) if entry_ids else []
    return [(winner.bookmaker, winner.label) for winner in winners]


def find_winners_by_signatures(entries, signatures):
    winners = []
    for bookmaker, label in signatures:
        winner = entries.filter(bookmaker=bookmaker, label=label).first() or entries.filter(label=label).first()
        if winner and winner.pk not in {entry.pk for entry in winners}:
...
def dashboard_period(request, bets):
    ...
    for bet in bets:
        years.add(timezone.localtime(bet_accounting_at(bet)).year)
...
def build_dashboard_context(request, **forms):
    ...
    all_bets = user_bets(request.user).select_related(
        'bankroll',
        'bankroll__entity',
        'entity',
        'extracted_freebet',
    ...
    available_freebets = user_freebets_queryset(request.user).filter(
        is_used=False,
        extraction_bet__isnull=True,
        simple_bet__isnull=True,
    )
    freebet_cycles_base = user_freebets_queryset(request.user).select_related('source_bet', 'extraction_bet', 'simple_bet')
    ...
    balance_before_period = total_initial_balance + sum(
        (
            ...
            and timezone.localtime(bet_accounting_at(bet)) < dashboard_filter['reference_date']
        ),
        start=Decimal('0.00'),
    )
...
def calculate_double_protection(early_profit, second_chance_profit, live_odd, commission):
    if (
        early_profit is None
        or second_chance_profit is None
        or live_odd is None
        or commission is None
        or second_chance_profit <= 0
        or live_odd <= 1
    ):
        return None
    payout_multiplier = (Decimal('1') + ((live_odd - Decimal('1')) * (Decimal('1') - (commission / Decimal('100')))))
    ...
    return {
        ...
        'worst': min(team_one_net, double_net),
    }
...
def user_freebets_queryset(user):
    return FreeBet.objects.filter(
        Q(owner=user)
        | Q(source_bet__bankroll__owner=user)
        | Q(source_bet__entity__owner=user)
        | Q(extraction_bet__entity__owner=user)
    ).distinct()


def source_freebet_from_post(user, post_data, existing_bet=None):
    freebet_id = (post_data.get('freebet_source') or '').strip()
    if not freebet_id:
        return None
    queryset = user_freebets_queryset(user).filter(pk=freebet_id)
    if existing_bet is not None:
        queryset = queryset.filter(Q(is_used=False) | Q(extraction_bet=existing_bet))
    else:
        queryset = queryset.filter(is_used=False, extraction_bet__isnull=True, simple_bet__isnull=True)
    return queryset.select_related('source_bet').first()
...
def index(request):
    if request.method == 'POST':
        ...
        if form_type == 'double_protection':
            double_errors = []
            entity_id = request.POST.get('double_entity')
            bankroll_id = request.POST.get('double_bankroll')
            entity = Entity.objects.filter(pk=entity_id, owner=request.user).first() if entity_id else None
            bankroll = Bankroll.objects.filter(
                pk=bankroll_id,
                owner=request.user,
            ).select_related('entity').first() if bankroll_id else None
            game = (request.POST.get('double_game') or '').strip()
            sport = (request.POST.get('double_sport') or 'Futebol').strip()
            competition = (request.POST.get('double_competition') or '').strip()
            event_date = event_date_from_post(request.POST, 'double_event_date')
            external_event_id = (request.POST.get('double_external_event_id') or '').strip()
            external_sport_key = (request.POST.get('double_external_sport_key') or '').strip()
            home_team = (request.POST.get('double_home_team') or '').strip()
            away_team = (request.POST.get('double_away_team') or '').strip()
            notes = (request.POST.get('double_notes') or '').strip()
            early_profit = decimal_from_post(request.POST, 'double_early_profit')
            second_chance_profit = decimal_from_post(request.POST, 'double_second_chance_profit')
            live_odd = decimal_from_post(request.POST, 'double_live_odd')
            commission = decimal_from_post(request.POST, 'double_commission')

            if entity is None:
                double_errors.append('Selecione uma entidade válida.')
            if bankroll is None:
            ...
            if commission is None or commission < 0 or commission > 100:
                double_errors.append('A comissão precisa ficar entre 0% e 100%.')

            calculation = None if double_errors else calculate_double_protection(
                early_profit,
                second_chance_profit,
                live_odd,
                commission,
            )
            ...
            protection_lines = [
                ...
                f'Se time 1 confirmar: {format_money(calculation["team_one_net"])}',
                f'Se empate/virada acontecer: {format_money(calculation["double_net"])}',
            ]
            if notes:
                protection_lines.extend(['Observações:', notes])
            ...
            SureBetEntry.objects.create(
                bet=bet,
                bankroll=bankroll,
                bookmaker=bankroll.display_name,
                label='Time 1 confirma',
                mode=SureBetEntry.Mode.BACK,
                odds=live_odd,
                effective_odds=calculation['payout_multiplier'].quantize(Decimal('0.01')),
                stake=calculation['stake'],
                liability=calculation['stake'],
                commission=commission,
            ...
            SureBetEntry.objects.create(
                bet=bet,
                bankroll=bankroll,
                bookmaker='Cenário protegido',
                label='Empate ou virada',
                mode=SureBetEntry.Mode.BACK,
                odds=Decimal('1.00'),
                effective_odds=Decimal('1.00'),
                stake=Decimal('0.00'),
                liability=Decimal('0.00'),
            ...
            messages.success(request, 'Proteção do duplo cadastrada com sucesso.')
            return redirect('dashboard:index')

        if form_type == 'freebet_extract':
            extraction_errors = []
            selected_source_freebet = source_freebet_from_post(request.user, request.POST)
            if (request.POST.get('freebet_source') or '').strip() and selected_source_freebet is None:
                extraction_errors.append('Selecione uma freebet disponível para vincular ou deixe o campo em branco.')
            outcomes = build_freebet_extract_payload(request.POST, selected_source_freebet)
            ...
            with transaction.atomic():
                ...
                if selected_source_freebet:
                    selected_source_freebet.is_used = True
                    selected_source_freebet.extraction_bet = bet
                    selected_source_freebet.save(update_fields=['is_used', 'extraction_bet'])
...
def edit_bet(request, pk):
    ...
    if is_protection_bet(bet):
        ...
        source_freebets = user_freebets_queryset(request.user).filter(
            Q(is_used=False) | Q(extraction_bet=bet),
        ).select_related('source_bet').distinct()
        errors = []
        balance_prompt = None
        ...
        if request.method == 'POST':
            ...
            if is_freebet_edit:
                selected_source_freebet = source_freebet_from_post(request.user, request.POST, existing_bet=bet)
                if (request.POST.get('freebet_source') or '').strip() and selected_source_freebet is None:
                    errors.append('Selecione uma freebet disponível para vincular ou deixe o campo em branco.')
                outcomes = build_freebet_extract_payload(request.POST, selected_source_freebet)
                game = (request.POST.get('freebet_game') or '').strip()
                sport = (request.POST.get('freebet_sport') or 'Futebol').strip()
                competition = (request.POST.get('freebet_competition') or '').strip()
                ...
                if not errors:
                    ...
                    with transaction.atomic():
                        ...
                        if winners:
                            apply_manual_protection_winners(
                                bet,
                                bet.surebet_entries.select_related('bankroll').all(),
                                winners,
                                request.user,
...
def settle_surebet(request, pk):
    ...
    if request.method == 'POST':
        entry_ids = request.POST.getlist('winner_entry')
        winners = list(entries.filter(pk__in=entry_ids)) if entry_ids else []
        if not winners:
            messages.error(request, 'Selecione pelo menos uma casa vencedora da arbitragem.')
            return render(
                request,
